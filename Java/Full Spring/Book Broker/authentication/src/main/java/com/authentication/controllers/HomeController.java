...
public class HomeController {
    ...
    public String borrow(Model model, HttpSession session) {
    	if (session.getAttribute("user_id") != null) {
            ...
            List<Book> notBorrowed = userServ.notBorrowed();
            model.addAttribute("books", notBorrowed);
            
            List<Book> borrowed = userServ.AllBorrower(thisUser);
            model.addAttribute("borrow", borrowed);
    ...
    public String borrwoed(@PathVariable("id")Long id, HttpSession session) {
    	Book book = userServ.findBook(id);
    	Long user_id1 = (Long) session.getAttribute("user_id");
        User thisUser = userServ.findUserById(user_id1);
    	book.setBorrow(thisUser);
    	userServ.updateBook(book);
    	return "redirect:/bookmarket";
    }
    
    @GetMapping("/bookmarket/return/{id}")
    public String notBorrowed(@PathVariable("id")Long id, HttpSession session) {
    	Book book = userServ.findBook(id);
    	Long user_id1 = (Long) session.getAttribute("user_id");
        User thisUser = userServ.findUserById(user_id1);
        book.setBorrow(null);
